...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $insert_query = "INSERT INTO championship (champ_name, category_id, start_date, end_date, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?)";
    $insert_stmt = $conn->prepare($insert_query);

    if ($insert_stmt) {
        $insert_stmt->bind_param("ssssss", $champ_name, $category_id, $start_date, $end_date, $start_time, $end_time);
        $insert_stmt->execute();
        $insert_stmt->close();
